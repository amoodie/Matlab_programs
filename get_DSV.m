...
%   Dietrich settling velocity wa
    ...
    for d = 1:length(D(:)) % for each grain size
        Da = ((con.rho_s - con.rho_f) * con.g * (D(d) ^ 3)) / (con.rho_f * (con.nu ^ 2));
        if Da <= 0.05; % use Stokes for Da <= 0.05, from Dietrich, 1982 paper
            wa(d) = 1.71e-4 * (Da^2); 
        elseif Da > 0.05 && Da <= 5e9; % use Dietrich, 1982 only if within valid range
            R_1 = (-3.76715) + (1.92944 * log10(Da)) - (0.09815 * (log10(Da)^(2.0))) - ...
                (0.00575 * (log10(Da)^(3.0))) + (0.00056 * (log10(Da)^(4.0)));
            R_2 = (log10(1 - ((1 - CSF) / 0.85))) - (((1 - CSF)^(2.3)) * tanh(log10(Da) - 4.6)) + ...
                (0.3 * (0.5 - CSF) * ((1 - CSF)^(2.0)) * (log10(Da) - 4.6));
        ...
        ws(d) = ((wa(d) * (con.rho_s - con.rho_f) * con.g * con.nu) / con.rho_f)^(1/3);
    end
